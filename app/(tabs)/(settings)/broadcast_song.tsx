...
export default function BroadcastOptionsScreen() {
    ...
    async function broadcast_song_number() {
        ...
        let number = verseInput.replace(/^0+/, '');
    ...
    const renderItem = ({ item }: ListRenderItemInfo<string>) => (
        <TouchableOpacity
            style={{
                ...
                borderRadius: 15,
                borderWidth: 3,
                borderColor: selectedHymnal == item ? Colors[theme]['primary'] : Colors[theme]['background'],
                alignItems: 'center',
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                if(selectedHymnal.length < 1)
                                    return;
                                if(verseInput.length < 1)
                                    return;

                                broadcast_song_number()
                            }}
                            disabled={selectedHymnal.length < 1 || verseInput.length < 1}
                        >
                            <Ionicons
                                name='send'
                                size={18}
